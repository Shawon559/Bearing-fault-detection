...
class MultiScaleAttentionFusion4Class(nn.Module):
    """Multi-scale attention fusion model for 4-class bearing fault classification."""
    def __init__(
        self,
        envelope_dim: int,
        fft_dims: list,
        ...
        hidden_dim: int = 64,
        fusion_dim: int = 128,
        num_classes: int = 4,  # 4 classes: Healthy, Inner, Outer, Cage
        dropout: float = 0.3,
    ):
        super().__init__()
...
def create_model_4class(sample_batch, hidden_dim=64, fusion_dim=128, num_classes=4, dropout=0.3):
    # Get dimensions from sample
    envelope_dim = sample_batch['envelope'].shape[1] if len(sample_batch['envelope'].shape) > 1 else sample_batch['envelope'].shape[0]
    fft_dims = [
        sample_batch['fft_scale1'].shape[1] if len(sample_batch['fft_scale1'].shape) > 1 else sample_batch['fft_scale1'].shape[0],
